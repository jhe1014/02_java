...
/**
 * 바이트 처리만 할 수 있는 이미지 파일을 입력받아서 이미지 파일로 출력하는 (stream으로만 작업하는) 이미지 복사 클래스
 * -------------------------------------------------------------------------------------------------------------------------
 * -- 입력 --
 ...
 * 4. write 작업 : node steam 메소드 write(byte[])
 * 
 * -- 정리 --
 * 5. 입력 node 닫기
 * 6. 출력 node 닫기
...
public class ImageCopy {
	...
	public static void main(String[] args) throws IOException {
		// 1. 입력 node stream : FileInputSream
		...
		// ===== 출력 node stream선언, 초기화 완료
		
		// 2. read 작업 : read(byte[]) 메소드
		//				  읽기 작업의 매개변수로 사용될 byte[]을 선언
		
		// (1) 바이트 배열 : buffer
		byte[] buffer = new byte[4096];
		
		// (2) read(byte[]) 메소드는 읽어들인 바이트 수를 정수로 리턴하는 메소드
	    // 	   읽은 데이터가 없으면 -1 리턴
		int readSize = 0;
		int reapeat = 0;
		...
		while((readSize = in.read(buffer)) != -1) {
			...
			// 화면에 작업 내용 출력
			System.out.printf("읽은 바이트 : %d\t, 반복 횟수 : %d%n", readSize, ++reapeat);
		}
