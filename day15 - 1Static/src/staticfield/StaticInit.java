...
/**
 * static 멤버(변수, 메소드)와 non-static 멤버(변수, 메소드)의 초기화 시점에 따른 참조를 구성하는 클래스
 * 
...
public class StaticInit {
	...
	/**
	 * static 필드인 count 변수와 non-static 필드인 memberCnt 변수의 값을 모두 출력하는 메소드
	 */
	...
	/** 
	 * 이 클래스의 count 값과 입력된 StaticInit 객체로부터 memberCnt 값을 출력
	 * @param init
	 */
	...
	/**
	 * 이 클래스의 클래스 변수(static 변수)인 count와 인스턴스 변수인 memberCnt를 더한 값을 리턴하는 메소드
	 * -----------------------------------------------------------------------------------------------------
	 * 1. 이 메소드는 static modifier가 붙지 않았으므로 non-static 메소드가 된다.
	 * 2. non-static 메소드인 이 메소드에서 static 필드인 count 값을 사용(참조)하려 할 때 아무런 제약없이
	 * 	  바로 사용 가능
